...
var Num = /** @class */ (function () {
    ...
    Object.defineProperty(Num.prototype, "base", {
        /**
         * The base that the number is in.
         */
        get: function () {
     ...
     * @private
     */
    Num.prototype.Convert = function (base, precision) {
         ...
         */
        var digit = this._number;
        var digitLog = Math.floor(Math.log(digit) / Math.log(this._base)) + 1;
        //If a number is less than one, it'll have zero digits, but the above code will output negative digits. This corrects it.
        if (digitLog < 0)
            digitLog = 0;
        var digits = [];
        var toAdd = [];
        for (var i = digitLog - 1; i > (-1 * precision) - 1; i--) {
            var number = Math.floor((digit / Math.pow(this._base, i)) % this._base);
            digit -= number * Math.pow(this._base, i);
            var digitStr = NumberToDigit(number);
            if (digitStr !== "0" || digits.length < digitLog) {
                digits.push.apply(digits, __spreadArray(__spreadArray([], toAdd), [digitStr]));
                toAdd = [];
            }
            else
                toAdd.push("0");
        ...
        if (digits.length < digitLog)
            digits.push.apply(digits, "0".repeat(digitLog - digits.length).split(""));
     ...
     */
    Num.prototype.toBase = function (base) {
     ...
     * @param precision {number} - is the maximum decimal places a decimal should have.
     */
    Num.prototype.toString = function (precision) {
        ...
        if (!this._cache.hasOwnProperty(this._base.toString() + "|" + precision.toString()))
     ...
     * @param precision {number} - is the maximum decimal places a decimal should have.
     */
    Num.prototype.toNumber = function (precision) {
        if (precision === void 0) { precision = 8; }
